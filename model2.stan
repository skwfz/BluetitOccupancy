...
model {
  ...
  for(j in 1:N_habitats){//priors for coefs
    habitat_coef[j] ~ normal(0,2);
  }
  for(i in 1:N_localities){
    occupied_prob[i] = inv_logit(sum(habitat_coef .* habitat_prop[i,]));
  }
  
  //Detection model
  duration_coef ~ normal(0,2);
  distance_coef ~ normal(0,2);
  forest_prop_coef ~ normal(0,2);
  a0 ~ normal(0,2);
  a1 ~ normal(0,2);
  b1 ~ normal(0,2);
  for(k in 1:N){
    detected_prob[k] = inv_logit(duration_coef*duration_minutes[k] + 
                                distance_coef*distance_traveled[k] + 
                                forest_prop_coef*forest_prop[k]) *
                       inv_logit(a0/2 + a1*cos(2*pi()*start_time[k]/24) + b1*sin(2*pi()*start_time[k]/24));
  }
...
generated quantities{
  ...
  for(i in 1:check_N){
    occupied_prob[i] = inv_logit(sum(habitat_coef .* habitat_prop[check_indices_habprop[i],]));
  }
  for(k in 1:check_N){
    detected_prob[k] = inv_logit(duration_coef*duration_minutes[check_indices_others[k]] + 
